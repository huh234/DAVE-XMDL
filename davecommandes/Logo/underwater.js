module.exports = async (context) => {
  ...
  if (!text || text.trim() === '') {
    return m.reply(`◈━━━━━━━━━━━━━━━━◈\n│❒ Yo, brain-dead moron, give me some text for the Underwater logo! Use *!underwater SomeText* or fuck off! 😡`);
  }
  ...
  try {
    ...
    const data = await fetchJson(`https://api.giftedtech.web.id/api/ephoto360/underwater?apikey=gifted&text=${encodedText}`);

    if (data && data.success && data.result && data.result.image_url) {
      const caption = `◈━━━━━━━━━━━━━━━━◈\n│❒ Here’s your damn *Underwater* logo, ${m.pushName}! Don’t waste my time again, you waterlogged prick! 😤\n` +
                     `📸 *Text*: ${cleanedText}\n` +
      ...
      await m.reply(`◈━━━━━━━━━━━━━━━━◈\n│❒ API’s being a bitch, no Underwater logo for you, loser! Try again later. 😒`);
    }
  } catch (error) {
